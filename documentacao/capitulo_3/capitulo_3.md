...
## **postgresql.conf**
O **postgresql.conf** e o principal arquivo de configuração do PostgreSQL, normalmente localizado em `/var/lib/pgsql/14/data/` (**$PGDATA**). Esse arquivo prescreve o comportamento para o cluster inteiro, ou seja, não é possível configurar comportamentos diferentes em relação a cada banco de dados. 

E interessante observar que a partir da versão **9.4** foi adicionado o **ALTER SYSTEM**, que permite alterar os valores de parâmetros sem modificar o **postgresql.conf**. Por isso é **recomendado a verificação dos valores do parâmetro do PostgreSQL seja sempre realizada pela view pg_settings**. Os parâmetros ali encontrados dividem-se em várias seções no arquivo:
...
- **max_connection**
  Número máximo de conexões suportadas pelo cluster. O valor inicial de max_connection é **100**.

- **work_mem**
  O work_men é o espaço utilizado para operações de **bitmap**, **hash**, **join** e **merge**.

- **maintenance_work**
  Esse parâmetro define o espaço usado pelo **VACUUM** e **CREATE INDEX**. O valor default é 64 MB.
...
SELECT 
  ...
  source,
  CASE context
    WHEN 'postmaster' THEN 'RP2CEQUIRES RESTART'
...
O **contexto** de cada variável informa-nos da necessidade ou não de reinicialização, abaixo consulta alternativa:
...
A autenticação no **cluster** é controlada por um arquivo de configuração chamado **pg_hba.conf**, armazenado no diretório de dados do cluster **$PGDATA** (HBA - host-based authentication (autenticação baseada em host)). Um arquivo **pg_hba.conf** padrão é instalado quando o diretório de dados é inicializado pelo **initdb**.
...
- **Local** <br/>
  Conexão usando soquetes de domínio **Unix**. Sem um registro de tipo, as conexões de soquete de domínio não são permitidas. Quando não é inserida a opção **-h**, o **PSQL** tenta conectar-se por esse método.
...
- **Database** <br/>
  Especifica quais databases o registro correspondente pode acessar. O valor **all** determina que qualquer database pode ser acessado. Múltiplos nomes podem ser fornecidos separados por vírgula, pode ser criado também um arquivo separado com nomes dos databases onde o arquivo pode ser especificado antes do nome do arquivo com um **@**.
...
- **Peer** <br/>
  Recupera o nome de usuário do sistema operacional cliente e verifica se este corresponde ao nome de usuário do database solicitado.
...
- **PAM** <br/>
  Realiza a autenticação usando o  serviço de Módulos de Autenticação Plug-in (PAM) fornecido pelo sistema operacional.
...
**Para que as configurações surtam efeito/sejam carregadas no cluster, é necessário forçar o cluster a ler novamente os arquivos postgresql.conf e pg_hba.conf, com o seguinte comando no Red Hat, CentOS, Oracle...:**

```bash
systemctl reload postgresql-14
```

Depois de configurado, precisamos realizar a conexão ao cluster, o usuário inicial é o **postgres**, o **owner** do cluster. O usuário postgres é gerado por default na criação do cluster, podendo ser realizado o primeiro acesso dessa forma:
